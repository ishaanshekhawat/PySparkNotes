...
# Demo - Operations - Stream-Batch Join (Demo Not working on pyspark 2.4.5, need to check on other versions)
#
...
.builder\
.master("local")\
.appName("Operation - stream-batch join")\
.getOrCreate()
...
companyDF = spark.read.csv("file:///home/talentum/Spark-S-Streaming_Demos/data/COMPANY.csv", header = True)

# Perform basic aggregation on our streaming DataFrame.
resultDF = initDF.groupBy("Name", F.year("Date"))\
.max("High")
resultDF = resultDF.join(companyDF, on = "Name", how = "inner")\
.select("FullCompanyName", "year(Date)", "max(High)")
